...
def trim(
    ...
    replace: bool = typer.Option(
        False, "--replace", "-r", help="Replace the original file instead of creating a new one."
    ),
):
    """
    ...
    if image_path.is_dir():
        ...
        for file_path in files_to_process:
            try:
                output_path = image_ops.trim_image(file_path, margin, replace)
        ...
        # Single file processing
        try:
            output_path = image_ops.trim_image(image_path, margin, replace)
